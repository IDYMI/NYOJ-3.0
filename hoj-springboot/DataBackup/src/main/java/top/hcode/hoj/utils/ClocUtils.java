...
public class ClocUtils {
     ...
     */
    public void getUserCodeLines(List<String> uidList, Date startTime)
            throws StatusFailException, StatusNotFoundException, IOException {

        List<String> codeUidList = getUserCodeList(uidList, startTime);

        // 发送指令
        if (!CollectionUtils.isEmpty(codeUidList)) {
            List<ClocVO> clocResult = sendClocCmd(codeUidList);
            ...
            for (ClocVO clocVO : clocResult) {
                ...
                // 一小时内最多只能生成一次
                if (userCloc == null || !userCloc.getTime().equals(day)) {
                    ...
                    if (sum != userCloc.getSum() && sum != 0L) {
                        ...
                        insertUserClocList.add(new UserCloc()
                                ...
                                .setTime(day)
                                .setJson(codeConfigJson.toString())
                                .setSum(clocResultDTo.getSum()));
                    }
                }
            }

            if (!CollectionUtils.isEmpty(insertUserClocList)) {
    ...
    public List<String> getUserCodeList(List<String> uidList, Date startTime) throws StatusFailException {
        QueryWrapper<Judge> judgeQueryWrapper = new QueryWrapper<>();

        judgeQueryWrapper.select("username", "uid", "language", "code", "submit_time", "status")
                .in(uidList.size() > 0, "uid", uidList)
                .eq("status", 0) // 正确代码
                .ge("submit_time", startTime)
        ...
        // 实际用户列表
        List<String> codeUidList = new ArrayList<>();
        ...
        for (Judge judge : judgeList) {
            futureTasks.add(new FutureTask<>(new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    ...
                    String day = DateFormatUtils.format(judge.getSubmitTime(), "yyyy-MM-dd-HH_mm_ss");
                    // 文件后缀
                    String suffix = contestFileManager.languageToFileSuffix(judge.getLanguage().toLowerCase());
                    // 创建用户对应的文件夹
                    String usercodeWorkDir = workDir + File.separator + uid;

                    codeUidList.add(uid);

                    File file = new File(usercodeWorkDir);

                    if (!file.exists() || file.listFiles() == null) { // 本地为空
                        FileUtil.mkdir(usercodeWorkDir);
                        System.out.println(usercodeWorkDir);
                    }

                    String outputName = usercodeWorkDir + File.separator + day + "." + suffix;

                    File file2 = new File(outputName);

                    if (!file2.exists() || file2.listFiles() == null) { // 存在则说明，该提交记录已经保存
                        FileWriter outfileWriter = new FileWriter(outputName);
                        outfileWriter.write(judge.getCode());

                        FileUtil.copy(usercodeWorkDir, workDir, true);
                    }
    ...
    public List<ClocVO> sendClocCmd(List<String> codeUidList) throws IOException {
        // 去重
        List<String> uidList = codeUidList.stream().distinct().collect(Collectors.toList());
        ...
        for (String uid : uidList) {
            futureTasks.add(new FutureTask<>(new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    ...
                    // 用户对应的文件夹
                    String usercodeWorkDir = codeDir + "/" + uid;
                    ...
                    try {
                        ...
                        // 构建命令并执行
                        String cmd = getCmdCommand(usercodeWorkDir, null);
